...
class SamPredictor:
    ...
    def set_torch_image(
        ...
        self.features= self.model.image_encoder(input_image, input_points, input_box, feature_map)
        self.is_image_set = True
    ...
    def predict_promptEncoder(
        ...
        sparse_embeddings, dense_embeddings, feature_map = self.predict_torch_promptEncoder(
            coords_torch,
            labels_torch,
            box_torch,
            mask_input_torch,
    ...
    def predict_maskDecoder(
        ...
        masks, iou_predictions, low_res_masks = self.predict_torch_maskDecoder(
    ...
    #     point_labels: Optional[np.ndarray] = None,
    #     box: Optional[np.ndarray] = None,
    #     text: Optional[str] = None,  # 新增 text 参数
    #     mask_input: Optional[np.ndarray] = None,
    #     multimask_output: bool = True,
    #     return_logits: bool = False,
    # ) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    #   """
    #   Predict masks for the given input prompts, including text prompt if provided.
    ...
    #     return_logits (bool): If true, returns un-thresholded masks logits
    #       instead of a binary mask.
    #     text (str or None): Optional text prompt to help guide the mask prediction.
    ...
    #   # Convert text to prompt embeddings if a text prompt is provided
    #   if text is not None:
    ...
    # #       mask_input_torch,
    # #       multimask_output,
    # #       text=text_prompt_embeddings,  # 传入文本嵌入，此时仍是str类型并未做任何改变
    # #       return_logits=return_logits,
    # #   )
    #   masks, iou_predictions, low_res_masks = self.predict_torch(
    ...
    #         multimask_output=multimask_output,  # 这里显式指定参数名
    #         text=text_prompt_embeddings,
    #         return_logits=return_logits,
    #     )
